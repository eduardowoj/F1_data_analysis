...
head(results.by.driver,15)

# How many races each driver won?
# This is a very polluted plot, because it considers every driver
# to ever win a race since the 50's

plt1 <- ggplot(data=results.by.driver, aes(x=driverRef,encoding="utf-8"))
...
# This is a much polluted plot, since this one consideres every
# driver that ever raced on F1
...
# of races by driver. Somewhat dirty implementation. The data frame contains both
# the drivers' surnames (as they're usually referred to, like Senna, Schumacher, Prost,
# Fangio, etc) and the driverRefs, an internal variable which, coincidentally,
# is also their surnames.

wins.by.winners <- summary(results.by.driver$surname,1000)
races.by.winners <- summary(races.by.driver$surname,1000)

win.percentage.by.driver = data.frame(wins.by.winners,races.by.winners)
#head(win.percentage.by.driver,20) # Uncomment to inspect the new data frame

# Add the driver's name column
win.percentage.by.driver$surname <- rownames(win.percentage.by.driver)
...
plt3 <- ggplot(data=top.drivers, aes(x=reorder(surname,-percentage), y=percentage))
...
plt4 <- ggplot(data=top.drivers.10races, aes(x=reorder(surname,-percentage), y=percentage))
